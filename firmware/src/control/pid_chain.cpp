 ...
 *
 */
float PIDChain::update(float target, float dT, PIDChainOverrideLevel ol) {
  float ret;

  switch (ol) {
  case PIDChainOverrideLevel::none:
    ret = link_h.update(target, dT);
    ret = link_m.update(ret,    dT);
    ret = link_l.update(ret,    dT);
    break;

  case PIDChainOverrideLevel::medium:
    ret = link_m.update(target, dT);
    ret = link_l.update(ret,    dT);
    break;

  case PIDChainOverrideLevel::low:
    ret = link_l.update(target, dT);
    break;

  case PIDChainOverrideLevel::bypass:
    ret = target;
    break;
