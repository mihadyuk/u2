...
{
  double wpWGS84[3][1] = {
      {deg2rad<mnrfp>(static_cast<double>(wp.x))},
      {deg2rad<mnrfp>(static_cast<double>(wp.y))},
      {static_cast<double>(wp.z)}};
  double wpLocalNED[3][1];

  geo2lv<double>(wpLocalNED, wpWGS84, currWGS84);
  localNE[0][0] = static_cast<mnrfp>(wpLocalNED[0][0]);
  localNE[1][0] = static_cast<mnrfp>(wpLocalNED[1][0]);
