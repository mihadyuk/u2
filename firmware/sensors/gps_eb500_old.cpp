...
static void parse_rmc(uint8_t *rmcbuf, mavlink_global_position_int_t *global_pos_struct){
  ...
  if (valid == 'A'){                              /* если координаты достоверны */
    ...
    gpstime = mktime(&gps_timp);
    chSysLock();
    TimeUsGps = gpstime * 1000000;  /* convert to uS */
    TimeUsGps += 1000000;           /* add difference between PPS and real time */
    setGlobalFlagI(GlobalFlags.time_gps_good);
    chSysUnlock();
    raw_data.gps_valid = TRUE;
  }
  else{
